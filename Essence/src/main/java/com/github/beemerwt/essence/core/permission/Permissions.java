...
public final class Permissions {
    ...
    public static boolean check(ServerPlayerEntity player, String node, int opLevelFallback) {
        if (isJailed(player)) return false;
        return REF.get().check(player, node, opLevelFallback);
    }

    public static boolean check(ServerPlayerEntity player, String node, OpLevel fallback) {
        return check(player, node, fallback.level);
    }

    public static boolean hasAnyChild(ServerCommandSource src, String prefix, OpLevel fallback) {
        if (isJailed(src)) return false;
        return REF.get().hasAnyChild(src, prefix, fallback.level);
    }

    public static boolean hasAnyChild(ServerPlayerEntity player, String prefix, OpLevel fallback) {
        if (isJailed(player.getCommandSource())) return false;
        return REF.get().hasAnyChild(player, prefix, fallback.level);
    }
    ...
    private static boolean isJailed(ServerPlayerEntity player) {
