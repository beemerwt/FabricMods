...
import com.github.beemerwt.essence.core.command.suggest.JailNameSuggester;
...
import com.github.beemerwt.essence.core.data.model.JailRecord;
import com.github.beemerwt.essence.core.permission.Perms;
import com.github.beemerwt.essence.core.text.Paginator;
...
public final class JailCommands {
    ...
    public static void register(CommandDispatcher<ServerCommandSource> d) {
        // /setjail <name>
        d.register(commandLiteral("essence", "setjail").requires(Perms.SET_JAIL::check)
            .then(argument("name", StringArgumentType.word())
                .executes(ctx -> {
                    ...
                    StoredLocation loc = new StoredLocation(sp.getEntityWorld().getRegistryKey(),
                        sp.getX(), sp.getY(), sp.getZ(), sp.getYaw(), sp.getPitch());
        ...
        // TODO: Register deljail

        // /jail <player> <jailname> [time] [reason...]
        d.register(commandLiteral("essence", "jail").requires(Perms.JAIL::check)
        ...
        // /unjail <player>
        d.register(commandLiteral("essence", "unjail").requires(Perms.UNJAIL::check)
            .then(argument("player", StringArgumentType.word()).suggests(PlayerSuggester.DATABASE)
                .executes(ctx -> {
                    var src = ctx.getSource();
                    var target = PlayerSuggester.getPlayer(ctx, "player");
        ...
        // /jailinfo <player>
        d.register(commandLiteral("essence", "jailinfo").requires(Perms.JAIL_INFO::check)
            .then(argument("player", StringArgumentType.word()).suggests(PlayerSuggester.DATABASE)
                .executes(ctx -> {
                    var src = ctx.getSource();
                    var target = PlayerSuggester.getPlayer(ctx, "player");
        ...
        // /jaillist [page]
        d.register(commandLiteral("essence", "jaillist").requires(Perms.JAIL_LIST::check)
    ...
    private static int execJail(
        ...
        // Teleport if online
        var world = Locations.resolveWorld(jailLoc);
