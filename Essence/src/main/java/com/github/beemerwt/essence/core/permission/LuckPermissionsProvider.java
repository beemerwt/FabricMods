...
public final class LuckPermissionsProvider implements PermissionsProvider {
    ...
    public boolean check(ServerPlayerEntity player, String node, int opLevelFallback) {
        try {
            ...
            return has != null ? has : player.hasPermissionLevel(opLevelFallback);
        } catch (Throwable t) {
    ...
    public boolean check(ServerCommandSource src, String node, int opLevelFallback) {
        ...
        return check(player, node, opLevelFallback);
    }
    ...
    public boolean hasAnyChild(ServerCommandSource src, String prefix, int opLevelFallback) {
        ServerPlayerEntity player = src.getPlayer();
        if (player == null) return src.hasPermissionLevel(opLevelFallback);
        return hasAnyChild(player, prefix, opLevelFallback);
    }
    ...
    public boolean hasAnyChild(ServerPlayerEntity player, String prefix, int opLevelFallback) {
        ...
        try {
            Object adapter = getPlayerAdapter.invoke(luckPerms, ServerPlayerEntity.class);
            Boolean has = (Boolean) hasPermission.invoke(adapter, player, wildcard);
            // If LP returns null/unknown, fall back to op-level
            return has != null ? has : player.hasPermissionLevel(opLevelFallback);
        } catch (Throwable t) {
            return player.hasPermissionLevel(opLevelFallback);
        }
